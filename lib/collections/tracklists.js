...
Tracklists.helpers({
  ...
  isExportable() {
    if (
      this.playDate &&
      (this.trackType === 'song' || this.trackType === 'talkingPoint')
    ) {
      return true
    }
  },
  ...
  trackType: {
    ...
    autoform: {
      ...
      options: [
        ...
        {
          label:
            'talkingPoint (tracking changes, but track is not exported in csv)',
          value: 'talkingPoint',
        },
        {
          label: 'showMeta (shows the description of the show)',
          value: 'showMeta',
        },
        { label: 'producerBio (shows producer bio)', value: 'producerBio' },
        {
          label: 'kuzuDefault (pulled from production status)',
          value: 'kuzuDefault',
        },
      ],
    },
